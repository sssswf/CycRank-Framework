...
def cycle_number(network):
    with open('../data/{}/Benchmarks/{}-nodes_CN.json'.format(network, network), "r") as f:
        CN_dict = json.load(f)
    return CN_dict
...
def cycle_edgeAvgCen(network):
    with open('../data/{}/Benchmarks/{}-edges_cen.json'.format(network, network), "r") as f:
        edge_dict = json.load(f)
    return edge_dict
def cycle_ranking(G,network):
...
def CycRak(G,p,index_dict):
    k = p * len(G.nodes())
    basic_cycle = nx.cycle_basis(G)
    cycle_id = cycle_ranking(G,network)
    seed_node = []
    for i in cycle_id:
        if len(seed_node) < k:
            tempt_index = {}
